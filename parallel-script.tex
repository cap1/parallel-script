...
\label{par:message_passing}
	...
	the threads or distributed programms sometimes will need to communicate with each other.
...
\label{par:input_&_output}
	In a parallel programm,
	one has to determine which parts of a programm are allowed to acces the standard input and output interfaces.
	In distributed memory programms only process 0 will have access to StandardIn
	and in a shared memory programm only the master thread
	or thread 0 will have acces to StandardIn.
